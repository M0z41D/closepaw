...
| Agent Behavior → Perception | Vertical radio list with outcome copy: Screen Transcript, Transcript + Image, Screen Image |
| Agent Behavior → Display Mode | Virtual Display toggle (Shizuku-gated; see `VirtualDisplayToggleGate` + `VirtualDisplayStatusMapper`) |
| Agent Behavior → Tools | Browser Script, Termux Shell, and other per-tool cards (`ToolSettingsCard`) |
...
| Permissions & Advanced | Debug mode, trace/data controls, app version |
