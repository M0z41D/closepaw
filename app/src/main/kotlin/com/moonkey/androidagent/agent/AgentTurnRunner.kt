        ...
        suspend fun executeTurn(
                ...
                val outcome =
                        try {
                                ...
                                Log.d(
                                        TAG,
                                        "Turn $turnNumber: Screen has ${snapshot.elements.size} elements"
                                )
                                ...
                                if (cancellationSignal.isCompleted || stopRequested.get()) {
                                        ...
                                        val navigationState =
                                                state.navigationState.advance(
                                                        snapshot = snapshot,
                                                        previousAction =
                                                                state.previousActionSignature
                                                )
                                        nextState =
                                                nextState.copy(navigationState = navigationState)
                                        ...
                                        if (loopWarning != null) {
                                                Log.w(
                                                        TAG,
                                                        "Turn $turnNumber loop warning: ${loopWarning.message}"
                                                )
                                        ...
                                        val stepReminder =
                                                buildStepReminder(turnNumber, stepDecision)
                                        if (stepReminder != null) {
                                                eventDispatcher.status(
                                                        "⚠️ Turn budget warning: approaching limit"
                                                )
                                        }
                                        ...
                                        val systemPrompt =
                                                com.moonkey.androidagent.agent.cognition.prompt
                                                                ...
                                                                visibleToolNames =
                                                                        config.allowedToolNames
                                                                                ?: services.toolRegistry
                                                                                        .getNames(),
                                                                llmBackend =
                                                                        services.config.llmBackend,
                                                                loopWarning = loopWarning,
                                                                systemReminders =
                                                                        listOfNotNull(stepReminder),
                                                                todos =
                                                                        services.sessionState.todos
                                                                                .get(),
                                                                scratchpadKeys =
                                        ...
                                        val userMessage =
                                                com.moonkey.androidagent.agent.cognition.prompt
                                                        .PromptUtils.buildUserMessage(promptContext)
                                        ...
                                        Log.d(
                                                TAG,
                                                "Turn $turnNumber: LLM response: ${result.content?.take(200)}..."
                                        )
                                        Log.d(
                                                TAG,
                                                "Turn $turnNumber: Tool calls: ${result.toolCalls.map { it.name }}"
                                        )
                                        ...
                                        val arbitration =
                                                turnPolicyEngine.arbitrateToolCalls(
                                                        result.toolCalls
                                                )
                                        ...
                                        if (arbitration.hasCompletionTool &&
                                        ...
                                        if (toolCallsToExecute.isNotEmpty()) {
                                                eventDispatcher.turnPhaseChanged(
                                                        turnId,
                                                        TurnPhase.EXECUTION
                                                )
                                                ...
                                                for (toolCall in toolCallsToExecute) {
                                                        ...
                                                        eventDispatcher.actionProposed(
                                                                toolCall.id,
                                                                toolCall.name,
                                                                ActionDescriptionFormatter.format(
                                                                        toolCall
                                                                )
                                                        )
                                                                ...
                                                                services.toolRouter.execute(
                                                                        ...
                                                                        onApprovalRequired = {
                                                                                ...
                                                                                try {
                                                                                        ...
                                                                                        Log.e(
                                                                                                TAG,
                                                                                                "Failed to emit approval required event",
                                                                                                e
                                                                                        )
                                                                                }
                                                                        }
                                                        ...
                                                        val (observation, observedSnapshot) =
                                                                when {
                                                                        toolResult is
                                                                        ...
                                                                        toolCall.name ==
                                                                                ...
                                                                                Log.d(
                                                                                        TAG,
                                                                                        "Skipping post-action capture for complete_task"
                                                                                )
                                                                                ToolObservation
                                                                                        .TextOutput(
                                                        ...
                                                        if (observedSnapshot != null) {
                                                                ...
                                                                Log.d(
                                                                        TAG,
                                                                        "Updated snapshot for subsequent tools: ${currentSnapshot.elements.size} elements"
                                                                )
                                                                eventDispatcher.screenCaptured(
                                        ...
                                        val completion =
                                                turnPolicyEngine.decideCompletion(
                                                        result,
                                                        arbitration
                                                )
                                        if (completion.shouldComplete) {
                                                ...
                                                Log.i(
                                                        TAG,
                                                        "Turn $turnNumber: Task marked as complete - $summary"
                                                )
