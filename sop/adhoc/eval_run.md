...
2. 用eval runner重新跑eval/config/aw_subset_group_234_round3.txt。`eval/.venv/bin/python eval/aw_bridge/runner.py --tasks-file eval/config/aw_subset_group_234_round3.txt`. 
    - 注意，过程中间,task之间可能会产生stall。比如不知道为什么丢掉accessibility权限,这时候会卡住。如果在一个任务等待卡很久,那你可能需要再去手动给 ClosePaw app grant一下accessibility permission，可能甚至需要你手动停止runner process再重跑一遍(这时候不要再重跑已经跑完的任务，缩小你的任务集合，跑剩余任务)。make sure跑完全部任务。
