 ...
 *
 * Screen capture: ImageReader (we own the surface, zero privilege). A11y tree:
 * AccessibilityService.windows filtered by displayId. Node actions:
 * AccessibilityNodeInfo.performAction (works across displays). Coordinate actions:
 * IInputManager.injectInputEvent via Shizuku. System buttons: KeyEvent injection with displayId.
 *
 * Lifecycle: start() → creates virtual display + ImageReader stop() → releases virtual display +
 * ImageReader
 *
...
class VirtualDisplayPlatform(
    ...
    companion object {
        ...
        private const val DISPLAY_FLAGS =
                ...
                0x8 or // VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY
                        0x40 or // VIRTUAL_DISPLAY_FLAG_SUPPORTS_TOUCH (undocumented but needed)
    ...
    private suspend fun performNodeClickAt(x: Int, y: Int): ActionResult {
        return withContext(Dispatchers.Main) {
            ...
            try {
                val node =
                        AccessibilityNodeFinder.findClickableNodeAtLocation(root, x, y)
                                ?: return@withContext ActionResult.Failure(
                                        "No clickable node at ($x,$y)"
                                )
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (component != null && shizukuAvailable) {
                    ...
                    // -W waits for launch to complete, good for ensuring window is ready
                    val cmd =
    ...
    /**
     * Get windows on our virtual display. Returns empty list if display invalid or windows
     * unavailable.
     */
    private fun getWindowsOnDisplay(): List<AccessibilityWindowInfo> {
        if (displayId == Display.INVALID_DISPLAY) return emptyList()
        return try {
            // API 33+ provides getWindowsOnAllDisplays() which returns windows keyed by displayId.
            // service.windows only returns the default display's windows!
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                ...
                if (allWindows != null && Log.isLoggable(TAG, Log.DEBUG)) {
                    ...
                    Log.d(TAG, "All windows (legacy): $windowSummary")
                }
    ...
    /**
     * Get the app window on our virtual display. Prefer TYPE_APPLICATION; fall back to any window
     * with a root.
     */
    ...
    private fun getRootOnDisplay(): AccessibilityNodeInfo? = getAppWindowOnDisplay()?.root

    /**
     * Create a MotionEvent targeting our virtual display. Uses hidden InputEvent.setDisplayId() via
     * HiddenApiBypass.
     */
