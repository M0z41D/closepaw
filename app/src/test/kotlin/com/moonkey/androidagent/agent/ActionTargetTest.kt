...
class ActionTargetTest {
    ...
    // --- ActionSignature integration ---

    @Test
    fun `signature click with element index`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"click","element_index":12}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:click:idx=12")
    }

    @Test
    fun `signature click with text`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"click","text":"Save"}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:click:text=save")
    }

    @Test
    fun `signature click with point`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"click","x":50,"y":75}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:click:xy=50,75")
    }

    @Test
    fun `signature click with no target`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"click"}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:click")
    }

    @Test
    fun `signature click with negative element index falls through`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"click","element_index":-1}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:click")
    }

    @Test
    fun `signature type with input_text uses target text`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"type","input_text":"hello","text":"Search"}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:type:text=search")
    }

    @Test
    fun `signature type with element index`() {
        val call = ToolCallRequest("1", "mobile_action",
            JSONObject("""{"action":"type","text":"hello","element_index":3}"""))
        assertThat(classifyActionSignature(call))
            .isEqualTo("mobile_action:type:idx=3")
    }
}
