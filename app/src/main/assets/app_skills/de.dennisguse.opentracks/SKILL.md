...
## Activity Type vs Track Name
Track names are user-chosen and do NOT indicate activity type. NEVER infer activity type from the track name — a track named "Trail Biking" could be "running" internally. The authoritative type is ONLY in the Edit screen.
...
## Date Labels
Track list uses relative labels (Today, Yesterday, Monday, etc.). Map each to an absolute date using the device date before filtering.
