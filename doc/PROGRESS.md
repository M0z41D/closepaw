...
- Password-in-file (vs OS keychain) chosen after libsecret/`secret-tool` route hit a locked GNOME keyring on the headless desktop session. Plain `~/secrets/closepaw/release.keystore.password` (mode 600) lives at the same trust boundary as the `.keystore` itself — no daemon dependencies, KISS, mirrors trivially via `scp`.
- R8 issues were pre-existing (not introduced by this branch) but masked because nobody had run a real `assembleRelease` recently. Surfaced during `publish-signing-keystore` verification; spun out as `publish-r8-release-fix` and gated `publish-release-workflow` on it so the workflow YAML only lands once it actually produces a working APK.

**Key files:**
- `app/build.gradle.kts`, `gradle.properties`, `scripts/release-build.sh`, `app/proguard-rules.pro`, `.gitignore`
- `README.md`, `SECURITY.md` (repo root)
- `.github/workflows/release.yml`
- `doc/dev/development.md` (Signed release builds section, R8 pitfalls)
