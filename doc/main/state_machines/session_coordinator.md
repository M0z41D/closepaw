...
| `Bound-Dead` | non-null, but next `submit` observes `SessionState.Shutdown` | transient — caller's `submit` immediately tears down and returns `SESSION_DEAD` (SessionCoordinator.kt:72-77) |

## SubmitResult

Sealed enum returned from `submit(text)` (SessionCoordinator.kt:266-275):

| Value | Meaning | When |
|---|---|---|
| `SENT` | Forwarded to session immediately as `Op.UserInput` | `currentSession.state ∈ {Created, Idle}` (default branch, SessionCoordinator.kt:82-85) |
| `QUEUED` | Appended to `pendingInputs`; will drain on next `Idle`/`Created` transition | `currentSession.state ∈ {Running, Paused, TakeoverPending}` (SessionCoordinator.kt:78-81) |
| `NO_SESSION` | No session bound | `currentSession == null` (SessionCoordinator.kt:69-70) |
| `SESSION_DEAD` | Session was already `Shutdown`; coordinator records `lastDeadSessionFileName` and tears down | `currentSession.state == Shutdown` (SessionCoordinator.kt:72-77) |

## CreateResult

Returned from `createAndSubmit(text, create)` (SessionCoordinator.kt:278-287):
...
- Only one `stateObserverJob` is alive per bound session (`stateObserverJob?.cancel()` before launching, SessionCoordinator.kt:237-239).
- `pendingInputs` only drains while `mutex` is held, and only while `session.state` is in `{Idle, Created}` (SessionCoordinator.kt:256-262).
- On `SESSION_DEAD`, the coordinator records the dead session's file name **before** teardown so callers can offer auto-reload via `consumeDeadSessionFileName()` (SessionCoordinator.kt:73-76, 201-205).
- `createAndSubmit` uses `tryLock` (non-suspending) so concurrent creation attempts return `LockBusy` instead of stacking up.
- `enqueue` is **not** mutex-guarded (SessionCoordinator.kt:155-159) — the doc-comment requires main-thread confinement. Same for `attachSession` / `detachSession` (SessionCoordinator.kt:166-195).
...
|---|---|
| `createAndSubmit` | Calls factory (which may create platform/services), assigns `currentSession`, launches state observer, submits first `Op.UserInput`, drains pending under lock (SessionCoordinator.kt:113-153) |
| `submit` (SENT path) | `session.submit(Op.UserInput(text))` |
| `submit` (SESSION_DEAD path) | Records dead-session file name, then `teardownLocked` (SessionCoordinator.kt:72-77, 228-234) |
| `clearSession` | `session.submit(Op.Shutdown)`, `delay(SHUTDOWN_GRACE_DELAY_MS=100)`, `teardownLocked` (SessionCoordinator.kt:210-225) |
| `detachSession` | Cancels observer, clears state — does **not** shut down the session (SessionCoordinator.kt:190-195) |
| stateObserver collector | On `Idle`/`Created`, calls `drainPending` which acquires the mutex and drains FIFO until empty or state changes (SessionCoordinator.kt:236-262) |

## Error / recovery paths

- Exceptions thrown by `clearSession`'s `Op.Shutdown` are swallowed (logged as warnings) so teardown still runs (SessionCoordinator.kt:216-218).
...
- A drain that observes a state change mid-loop simply breaks; remaining inputs stay queued for the next `Idle`/`Created` transition (SessionCoordinator.kt:256-262).
...
- `createAndSubmit`'s `Aborted` path clears all pending inputs (SessionCoordinator.kt:127-134). Any inputs queued via `enqueue` while a creation was in progress are silently dropped.
